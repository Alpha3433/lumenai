...
const ExamplesListView: React.FC<ExamplesListViewProps> = ({
  ...
  const featuredCompanies = companies.filter(company => ['tesla', 'apple', 'amazon'].includes(company.id));
  const otherCompanies = companies.filter(company => !['tesla', 'apple', 'amazon'].includes(company.id));
  console.log('Featured companies:', featuredCompanies);
  return <motion.main className="flex-1 w-full mx-auto px-4 py-16" /* Decreased padding-top back to py-16 */ initial={{
    opacity: 0
  }} animate={{
    opacity: 1
  }} transition={{
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12 max-w-3xl mx-auto">
          ...
          delay: 0.1
            ...
            <h1 className="text-4xl font-bold tracking-tight mb-4 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600">Industry-Leading Business Reports</h1>
            <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">Dive Into Actionable Insights &amp; Data-Driven Strategies from Top Performers</p>
          </motion.div>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-8">
          {featuredCompanies.map((company, index) => <motion.div key={company.id} custom={index} initial="hidden" animate="visible" variants={fadeInUpVariants} className="h-full">
              <ExampleCard company={company} onSelect={onSelectCompany} />
            </motion.div>)}
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mb-12">
          {otherCompanies.map((company, index) => <motion.div key={company.id} custom={index + 3} initial="hidden" animate="visible" variants={fadeInUpVariants} className="h-full">
              <ExampleCard company={company} onSelect={onSelectCompany} />
            </motion.div>)}
        </div>
