...
const ProcessSteps: React.FC<ProcessStepsProps> = ({
  ...
  return <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{
    ...
    margin: "-50px"
      ...
      {steps.map((step, index) => <motion.div key={index} variants={childVariants} className="flex gap-4 group">
          <div className="relative flex-shrink-0">
            <motion.div className="flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold text-lg shadow-lg" whileInView={{
          scale: [1, 1.2, 1],
          transition: {
            delay: index * 0.2,
            duration: 0.5
          ...
          once: true
            ...
            {index < steps.length - 1 && <div className="absolute left-1/2 top-14 bottom-0 w-1 h-[calc(100%-42px)] -translate-x-1/2 bg-gradient-to-b from-purple-300 to-blue-300 dark:from-purple-900/50 dark:to-blue-900/30"></div>}
          </div>
