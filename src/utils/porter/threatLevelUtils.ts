...
export const determineThreatLevel = (text: string): ForcesLevel => {
  ...
  if (/high[\s:\.]+|strong|significant|substantial|intense|considerable/i.test(lowerText)) return 'High';
  if (/medium[\s:\.]+|moderate|average|balanced|fair/i.test(lowerText)) return 'Medium';
  if (/low[\s:\.]+|minimal|limited|negligible|small|weak/i.test(lowerText)) return 'Low';
  ...
  return 'Medium'; // Default to Medium if no clear indicators or if tied
};
